...
function deepParse(chunk, state, keepFunctionCalls, skipEmptyValues) {
  ...
  for (; state.index < chunkLength; state.index++) {
    ...
    if ((isBeginningOfLine || !tempString) && isWhitespace(character)) {
      continue;
    }
    ...
    if (character === CHAR_NEWLINE) {
      if (!currentKey && tempString) {
        if (parsingKey) {
          if ((isFunctionCall(tempString) && !keepFunctionCalls) || isSpecialFunctionCall(tempString)) {
            continue;
          } else {
      ...
      if (tempString || (currentKey && !skipEmptyValues)) {
        addValueToStructure(out, currentKey, trimWrappingQuotes(tempString.trim()).trim());
    ...
    if (character === CHAR_BLOCK_START) {
      ...
      } else if (isSpecialFunctionCall(currentKey)) {
      ...
      if (isKeyword(tempString)) {
        ...
        } else if (tempString === KEYWORD_IF || tempString === KEYWORD_ELSE) {
...
function skipIfStatement(chunk, state) {
  ...
  var initiatedCommentSequence = false;
  ...
  for (var max = chunk.length; state.index < max; state.index++) {
    ...
    if (hasFoundAStatementWithoutBraces) {
      ...
      if (!hasFoundTheCurlyBraces && (character === CHAR_SLASH || initiatedCommentSequence)) {
        if (initiatedCommentSequence && (character === CHAR_SLASH || character === CHAR_ASTERISK) ) {
          skipComment(chunk, state, '/' + String.fromCharCode(character));
          initiatedCommentSequence = false
        } else if (character === CHAR_SLASH) {
          initiatedCommentSequence = true;
        }
        continue
      } else if (character === CHAR_BLOCK_START) {
...
function parseDependencyClosure(chunk, state) {
  ...
  for (; state.index < chunk.length; state.index++) {
    ...
    if (isStartOfComment(currentKey)) {
      skipComment(chunk, state, currentKey)
      currentKey = '';
    }
...
function skipComment(chunk, state, stringStart) {
  var commentText = stringStart;
      for (state.index = state.index + 1; state.index < chunk.length; state.index++) {
...
function isKeyword(string) {
  return string === KEYWORD_DEF || string === KEYWORD_IF || string === KEYWORD_ELSE || string === KEYWORD_TASK;
}
...
function isSpecialFunctionCall(string) {
  var lastDot = string.lastIndexOf('.');
  return lastDot !== -1 && string.slice(lastDot + 1) === KEYWORD_FOREACH;
}
